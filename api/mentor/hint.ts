...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        const systemPrompt = `Сен тәжірибелі, мейірімді бағдарламалау менторысың (ұстаз). Студентке код жазу тапсырмасында көмектесу керек. Бұл жолы толық дұрыс жауапты берместен, студентке нақты көмектесуің керек. Қажет болса формуланы, маңызды алгоритм қадамдарын немесе кодтық нұсқауларды көрсет. МАҢЫЗДЫ: көптеген оқушылар қалай функцияларды жазуды әлі білмегендіктен, мысалдарда немесе көмекші кодтарда "def" (функция құру) қолданба. Тек қарапайым циклдерді (for/while), шарттарды (if/else) және енгізу/шығаруды (input/print) қолдан. Студенттің қатесін нақты түсіндір және оны қалай түзеуге болатынын көрсет. Тіл: Қазақша.`;
